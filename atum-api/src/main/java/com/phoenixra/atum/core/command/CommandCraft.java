...
public abstract class CommandCraft implements CommandBase {
     ...
     */
    CommandCraft(@NotNull final AtumPlugin plugin,
    ...
    protected final void handleCommand(@NotNull final CommandSender sender,
        ...
        if (args.length > 0) {
            for (CommandBase subcommand : this.getSubcommands()) {
                ...
                ((CommandCraft) subcommand).handleCommand(sender, Arrays.copyOfRange(args, 1, args.length));
        ...
        if (args.length >= 2) {
            ...
            for (CommandBase subcommand : getSubcommands()) {
                ...
                if (args[0].equalsIgnoreCase(subcommand.getCommandName())) {
                    command = (CommandCraft) subcommand;
                }
            }
